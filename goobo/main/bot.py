...
def hint(reply_to, command_str):
    ...
    if command_str.startswith("--add "):
        ...
        try:
            jiyi.save()
            send_message(reply_to, "Hint '{}' has been created successfully. Use {}hint {} to check the message out.".format(hint, settings.COMMAND_PREFIX, hint))
        except IntegrityError:
            send_message(reply_to, "Hint '{}' has already exist, please be creative. Use {}hint {} to check the message out.".format(hint, settings.COMMAND_PREFIX, hint))
        return
    # query hint
    hint = command_str
    try:
        ...
        send_message(reply_to, "No such hint, you could request '{}' to be added by: {}hint --add {} <message>".format(hint, settings.COMMAND_PREFIX, hint))
